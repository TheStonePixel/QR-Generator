...
const EC_FORMAT_BITS = { M: 0b00, L: 0b01, H: 0b10, Q: 0b11 };

function formatInfoBits(ecLevel, mask) {
    const data = (EC_FORMAT_BITS[ecLevel] << 3) | mask; // 5 bits
    let bits = data << 10; // append 10 zero bits

    // divide by generator 0x537 (degree 10) to get remainder
    for (let i = 14; i >= 10; i--) {
...
const FORMAT_INFO = (() => {
    ...
    for (const ec of ["L", "M", "Q", "H"]) {
        table[ec] = [];
        for (let mask = 0; mask < 8; mask++) table[ec][mask] = formatInfoBits(ec, mask);
    }
    return table;
})();
...
function writeFormatInfo(matrix, ecLevel, maskIdx) {
    ...
    const bits = FORMAT_INFO[ecLevel][maskIdx];
...
function placeData(matrix, data) {
    ...
    for (let col = size - 1; col > 0; col -= 2) {
        // Skip the vertical timing column at position 6
        if (col === 6) col--;
...
function toSVG(qr, options = {}) {
    const { margin = 4, foreground = "#000000", background = "#ffffff", width = null, height = null } = options;
...
function toCanvas(qr, canvas, options = {}) {
    const { margin = 4, scale = 4, foreground = "#000000", background = "#ffffff" } = options;
